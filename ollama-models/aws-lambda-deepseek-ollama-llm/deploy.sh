...
AWS_ACCOUNT_ID=$(aws sts get-caller-identity --profile sunbirdai --query Account --output text)
...
aws ecr get-login-password --profile sunbirdai --region ${AWS_REGION} | docker login --username AWS --password-stdin ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com

# Step 2: Ensure ECR Repository Exists
echo "Checking if ECR repository exists..."
aws ecr describe-repositories --profile sunbirdai --repository-names ${ECR_REPO_NAME} --region ${AWS_REGION} >/dev/null 2>&1

if [ $? -ne 0 ]; then
    echo "ECR repository not found. Creating..."
    aws ecr create-repository --profile sunbirdai --repository-name ${ECR_REPO_NAME} --region ${AWS_REGION}
else
